...
def calculate_minervini_rs(equity_df):
    ...
    for i, code in enumerate(codes):
        if i % 50 == 0 and i > 0: time.sleep(0.5) 
        try:
            ...
            if len(df_hist) < 240:
                ...
                continue
            close = df_hist['Close']
            price_now = float(close.iloc[-1]); price_21d = float(close.iloc[-21]); price_63d = float(close.iloc[-63])
            price_126d = float(close.iloc[-126]); price_189d = float(close.iloc[-189]); price_240d = float(close.iloc[-240])
            
            weighted_ret = ((price_now/price_63d - 1) * 0.4 + (price_now/price_126d - 1) * 0.2 + (price_now/price_189d - 1) * 0.2 + (price_now/price_240d - 1) * 0.2)
            scores.append({'itemcode': code, 'weighted_return': weighted_ret, '1m_ret': (price_now/price_21d)-1, '3m_ret': (price_now/price_63d)-1, '1y_ret': (price_now/price_240d)-1})
        except:
            scores.append({'itemcode': code, 'weighted_return': None, '1m_ret': None, '3m_ret': None, '1y_ret': None})
            
    valid_scores = pd.DataFrame(scores).dropna(subset=['weighted_return']).copy()
    valid_scores['RS_Rating'] = (valid_scores['weighted_return'].rank(pct=True) * 99).apply(lambda x: int(round(x)))
    
    result_df = pd.merge(equity_df, valid_scores[['itemcode', '1m_ret', '3m_ret', '1y_ret', 'RS_Rating']], on='itemcode', how='inner').sort_values(by='RS_Rating', ascending=False)
...
def post_to_blogger(title, html_content):
    blog_id = os.environ.get('BLOGGER_BLOG_ID'); client_id = os.environ.get('BLOGGER_CLIENT_ID')
    client_secret = os.environ.get('BLOGGER_CLIENT_SECRET'); refresh_token = os.environ.get('BLOGGER_REFRESH_TOKEN')
    ...
    service.posts().insert(blogId=blog_id, body={"kind": "blogger#post", "title": title, "content": html_content}, isDraft=False).execute()
...
def export_data(df, bm_1m, bm_3m, bm_1y):
    ...
    for col in ['1개월', '3개월', '1년']:
        html_df[col] = (html_df[col] * 100).round(2).astype(str) + '%'
    
    # 1. [수정] 종목코드 색상을 이미지와 같은 푸른색(#3498db)으로 적용
    ...
    # 3. [수정] 설명 박스 왼쪽의 푸른 선(border-left) 디자인 적용
    html_content = f"""
    <div class="etf-container" style="font-family: 'Helvetica Neue', Arial, sans-serif; line-height: 1.6; color: #333; max-width: 100%; overflow-x: auto; margin-bottom: 30px;">
        ...
        <div class="description" style="font-size: 0.95em; color: #7f8c8d; margin-bottom: 15px; padding: 15px; background-color: #f8f9fa; border-radius: 5px; border-left: 5px solid #3498db;">
        ...
        <style>
            .etf-table {{ width: 100%; border-collapse: collapse; background-color: #ffffff; font-size: 0.9em; }}
            .etf-table th, .etf-table td {{ padding: 12px 15px; text-align: center; border-bottom: 1px solid #e0e0e0; }}
            .etf-table th {{ background-color: #f8f9fa; color: #2c3e50; font-weight: 600; white-space: nowrap; }}
            .etf-table td:nth-child(2) {{ text-align: left; }} /* 종목명은 좌측 정렬 */
